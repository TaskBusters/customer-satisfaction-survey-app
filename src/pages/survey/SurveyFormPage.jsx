...
import { isAgeValid, isEmailValid } from "../../survey/surveyUtils.js";
  ...
  const handleSubmit = async (e) => {
    ...
    surveyFields.forEach((field) => {
      ...
      if (
        ...
        field.conditionalRequired.skipValues?.includes(
          answers[field.conditionalRequired.dependsOn]
        )
      ) {
        return;
      }
      ...
      if (field.type === "matrix") {
        const missingRows = field.rows.filter(
          (row) => !answers[field.name] || !answers[field.name][row.name]
        );
        ...
        !answers[field.name] ||
        answers[field.name].toString().trim() === ""
      ) {
        fieldError = "Please select an option";
      }
    ...
    if (answers.email && !isEmailValid(answers.email)) {
    ...
    try {
      ...
      if (res.ok) {
        setToastMsg(
          isEditMode ? t("survey.updateSuccess") : t("survey.submitSuccess")
        );
        ...
        setToastMsg(
          isEditMode ? t("survey.updateFailed") : t("survey.submissionFailed")
        );
        setToastColor("bg-red-600/90 text-white");
        setShowToast(true);
        setSubmitting(false);
