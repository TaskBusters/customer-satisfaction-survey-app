...
export default function AdminProfileSecurityPage() {
  ...
  const handleCreateAdmin = async (formData) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/auth/admin/create-account`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteAdmin = async (adminEmail) => {
    ...
    try {
      ...
      setNotification({
        message:
          "Failed to delete admin account: " + (err.message || "Unknown error"),
        type: "error",
      });
      setTimeout(() => setNotification({ message: "", type: "success" }), 3000);
    } finally {
  ...
  const getRoleDescription = (role) => {
    ...
    const descriptions = {
      ...
      surveyadmin:
        "Full control over survey and data, cannot manage admin roles",
      "survey admin":
        "Full control over survey and data, cannot manage admin roles",
    ...
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <main className="flex-1 p-10">
        ...
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800">
          <strong>Note:</strong> Only System Administrators can access this page
          to manage admin roles and view activity logs.
        </div>
