    ...
    try {
      ...
      if (res.ok) {
        await logAdminAction(
          user.email,
          user.fullName,
          `Added new FAQ: "${newFaq.question}"`
        );
        const data = await fetch(`${API_BASE_URL}/api/admin/faqs`).then((r) =>
          r.json()
        );
        setFaqs(data);
    ...
    try {
      ...
      if (res.ok) {
        await logAdminAction(
          user.email,
          user.fullName,
          `Edited FAQ: "${oldFaq?.question || "FAQ"}" → "${
            editingFaq.question
          }"`
        );
    ...
    try {
      ...
      if (res.ok) {
        await logAdminAction(
          user.email,
          user.fullName,
          `Deleted FAQ: "${faqToDelete?.question || "FAQ"}"`
        );
...
export default function AdminHelpFeedbackPage() {
  ...
  // Reset to page 1 when feedback list changes (e.g., after reload)
  useEffect(() => {
    setCurrentPage(1);
  }, [feedback]);
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    try {
      ...
      if (res.ok) {
        setFeedback(
          feedback.map((f) => (f.id === id ? { ...f, status: newStatus } : f))
        );
        await logAdminAction(
          user.email,
          user.fullName,
          `Updated feedback status from ${
            feedbackItem?.status || "New"
          } to ${newStatus} for feedback ID ${id}`
        );
