...
export default function AdminNotificationsPage() {
  ...
  const fetchData = async (page) => {
    ...
    try {
      ...
      if (!totalItems) {
        try {
          const fullRes = await fetch(`${API_BASE_URL}/api/admin/notifications?limit=100000`, {
            headers: { Authorization: `Bearer ${authToken}` },
          })
          if (fullRes.ok) {
            const fullData = await fullRes.json()
            const fullList = fullData.data || fullData
            totalItems = Array.isArray(fullList) ? fullList.length : notificationsData.length
          } else {
  ...
  const deleteNotification = async (id) => {
    ...
    try {
      ...
      setTotalCount((prevCount) => {
        ...
        setTotalPages(Math.max(1, Math.ceil(newCount / NOTIFICATIONS_PER_PAGE)))
        return newCount
      })
  ...
  useEffect(() => {
    const notificationInterval = setInterval(() => {
      ...
      if (currentPage === 1) {
        ...
        fetch(`${API_BASE_URL}/api/admin/notifications?page=1&limit=${NOTIFICATIONS_PER_PAGE}`, {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
        })
          ...
          .then((data) => {
            ...
            setTotalPages(Math.ceil((data.total || notificationsData.length) / NOTIFICATIONS_PER_PAGE))
          })
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex">
        ...
        <main className="flex-1 p-10 text-center">
          ...
          <p className="text-gray-600">You do not have permission to view notifications.</p>
        </main>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <main className="flex-1 p-10 flex flex-col">
        <h1 className="text-3xl font-bold mb-6">Notifications ({totalCount} total)</h1>
          ...
          <div className="bg-white rounded-lg shadow p-6 flex-1 flex flex-col">
            ...
            <div className="space-y-3 flex-1 overflow-y-auto pr-4">
              {loading && totalCount > 0 ? (
                <p className="text-gray-500 text-sm">Loading page {currentPage}...</p>
              ) : notifications.length === 0 ? (
                <p className="text-gray-500 text-sm">No notifications found for this page.</p>
              ) : (
                    ...
                    <div className="flex-shrink-0 text-right flex flex-col items-end gap-2">
                      ...
                      {confirmDeleteId === notif.id ? (
                        <div className="flex gap-1">
                          ...
                          >
                            {deletingId === notif.id ? "Deleting..." : "Confirm"}
                          </button>
                          <button
                            type="button"
                            onClick={(e) => {
