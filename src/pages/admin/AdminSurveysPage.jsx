                  ...
                  {sectionFields.map((field) => (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        {field.instruction && <p className="text-sm text-gray-600 mt-2 italic">{field.instruction}</p>}
...
function FieldEditModal({ field, onSave, onCancel, onChange }) {
  ...
  const handleRowLabelChange = (index, newLabel) => {
    const updatedRows = (field.rows || []).map((row, idx) =>
      idx === index ? { ...row, label: newLabel } : row
    )
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow-lg w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto relative">
        ...
        <h2 className="text-2xl font-bold mb-4 text-blue-700 pr-8">Edit Question</h2>
