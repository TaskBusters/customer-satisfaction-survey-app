...
// ⭐ MODIFIED LOGS MODAL COMPONENT START ⭐
// ⭐ MODIFIED LOGS MODAL COMPONENT START ⭐
function LogsModal({ open, onClose, logs }) {
  ...
  const uniqueActionLabels = [
    ...new Set(logs.map((l) => getActionFilterLabel(l.action))),
  ].sort();
  ...
  const getPageNumbers = () => {
    ...
    return [...new Set(pageNumbers)].filter(
      (p) =>
        p !== "..." ||
        (p === "..." && pageNumbers.indexOf(p) !== pageNumbers.lastIndexOf(p))
    );
  };
  ...
  return (
    <div className="fixed z-40 inset-0 bg-black/25 flex items-center justify-center">
      <div className="bg-white rounded-lg p-6 w-full max-w-4xl shadow-xl relative">
        ...
        <h2 className="font-bold mb-4 text-xl border-b pb-2">
          All Admin Activity ({filteredLogs.length} entries)
        </h2>
        ...
        <ul className="max-h-96 overflow-y-auto text-sm space-y-2 mb-4 pr-2">
          {currentLogs.length === 0 ? (
            <li className="text-gray-500 py-8 text-center bg-gray-50 rounded">
...
export default function AdminDashboard() {
  ...
  const calculateSatisfactionDistribution = () => {
    ...
    allResponses.forEach((r) => {
      ...
      else if (satisfaction >= 1 && satisfaction < 2)
        satisfactionRanges["1-2"]++;
      else if (satisfaction >= 2 && satisfaction < 3)
        satisfactionRanges["2-3"]++;
      else if (satisfaction >= 3 && satisfaction < 4)
        satisfactionRanges["3-4"]++;
      else if (satisfaction >= 4 && satisfaction <= 5)
        satisfactionRanges["4-5"]++;
    });
    ...
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <main className="flex-1 p-8">
              ...
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <p className="text-3xl font-bold text-green-600 mt-2">
                  {stats?.avgSatisfaction
                    ? Number.parseFloat(stats.avgSatisfaction).toFixed(1)
                    : "—"}
                </p>
              </div>
              <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="bg-white border rounded p-4 w-full">
          ...
          <button
            className="mt-4 underline text-blue-500 text-sm hover:text-blue-600"
