        ...
        <div key={rowIdx} className="bg-white shadow rounded-xl mb-8 p-4 sm:p-6 lg:p-8 w-full max-w-full">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 md:gap-6 items-end w-full">
            {columns.map((col, colIdx) => (
              <div key={colIdx} className="flex flex-col items-center justify-start w-full h-full">
                {col.emoji && (
                  <span className="text-3xl sm:text-4xl mb-1 h-10 sm:h-12 flex items-center justify-center">
                    {col.emoji}
                  </span>
                )}
                <span
